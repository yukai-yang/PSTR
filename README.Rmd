...
You can, for example, run the wild bootstrap and the wild cluster bootstrap with parallel computation as follows.

```{r lintest3, eval=FALSE}
...
```
It can take a long time to run the bootstrap. This functionality is intended for users who run the package on machines with many cores and sufficient memory.

Parallel execution is implemented via the future and `future.apply` backend. No manual cluster initialisation is required. The option `PSTR.future.globals.maxSize` controls the maximum total size of objects that may be exported to each parallel worker.
